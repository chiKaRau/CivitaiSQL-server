...
public interface Models_Table_Repository
        ...
        @Query(value = """
                        SELECT
                          CASE WHEN local_path LIKE '\\\\%%' THEN '\\\\'
                               ELSE SUBSTRING(local_path, 1, 1) END AS drive,
                          SUBSTRING_INDEX(
                              SUBSTRING(local_path, LOCATE(:path, local_path) + LENGTH(:path)),
                              '\\\\', 1
                          ) AS directory,
                          COUNT(*)                                         AS file_count,
                          MIN(created_at)                                  AS dir_created_at,
                          MAX(updated_at)                                  AS dir_modified_at,
                          AVG(COALESCE(my_rating, -1))                     AS dir_rating_avg,
                          MAX(COALESCE(my_rating, -1))                     AS dir_rating_max
                        FROM models_table
                        WHERE LOCATE(:path, local_path) > 0
                          AND SUBSTRING(local_path, LOCATE(:path, local_path) + LENGTH(:path)) <> ''
                          AND SUBSTRING_INDEX(
                                SUBSTRING(local_path, LOCATE(:path, local_path) + LENGTH(:path)),
                                '\\\\', 1
                              ) <> ''
                        GROUP BY drive, directory
                        """, nativeQuery = true)
        List<Object[]> findVirtualDirectoriesAgg(@Param("path") String path);
